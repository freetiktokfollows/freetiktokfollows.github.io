...
function omega(LOL) {
  ...
  TESTOWANIEFLICKA = setInterval(function() {
    if (!ZALADOWANE) {
      ...
      var msgs = ["Looping through TikTok servers...", "Safely connecting with TikTok_" + chance.integer({min: 3, max: 333}) + ".DB()...", "Looking for " + LOL + " user...", "Downloading TikTok profile data..."];
...
function no() {
  ...
  COFNIETE = true;
    ...
    $("#igpro .label").text("Looping through TikTok servers...")
